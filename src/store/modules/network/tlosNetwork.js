...
const includedInTokens = (tokens) => (token) => tokens.some(t => compareToken(token, t));
export class TlosNetworkModule extends VuexModule.With({ namespaced: "tlosNetwork/" }) {
    constructor() {
        super(...arguments);
        this.tokenBalances = [];
    }
    ...
    get balance() {
        return ({ contract, symbol }) => {
            return this.balances.find(x => compareString(x.symbol, symbol) && compareString(x.contract, contract));
        };
    }
    get isAuthenticated() {
            ...
            return new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
                for (let i = 0; i < maxPings; i++) {
                    ...
                    const allBalancesDifferent = originalBalances.every(balance => {
                        var _a;
                        return ((_a = newBalanceArray.find(b => compareString(b.symbol, balance.symbol))) === null || _a === void 0 ? void 0 : _a.balance) !== balance.balance;
                    });
    ...
    transfer({ to, amount, id, memo }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!dirtyReserve)
                throw new Error("Failed finding dirty reserve");
    ...
    fetchBulkBalances(tokens) {
        return __awaiter(this, void 0, void 0, function* () {
            const balances = yield Promise.all(tokens.map((token) => __awaiter(this, void 0, void 0, function* () {
                const balance = yield getBalance(token.contract, token.symbol, token.precision);
                return Object.assign(Object.assign({}, token), { balance: asset_to_number(new Asset(balance)) });
            })));
            return balances;
        });
    }
    getBalances(params) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!params || ((_a = params === null || params === void 0 ? void 0 : params.tokens) === null || _a === void 0 ? void 0 : _a.length) == 0) {
                ...
                const equalisedBalances = bonusTokens.tokens.map(tokenBalanceToTokenBalanceReturn);
                const merged = _.uniqWith([...directTokens, ...equalisedBalances], compareToken);
            ...
            if (params === null || params === void 0 ? void 0 : params.slow) {
                ...
                const equalisedBalances = bulkTokens.tokens.map(tokenBalanceToTokenBalanceReturn);
                this.updateTokenBalances(equalisedBalances);
                const missedTokens = _.differenceWith(tokensAskedFor, equalisedBalances, compareToken);
                const remainingBalances = yield this.fetchBulkBalances(missedTokens);
                this.updateTokenBalances(remainingBalances);
                return [...equalisedBalances, ...remainingBalances].filter(includedInTokens(tokensAskedFor));
            }
            const [directTokens, bonusTokens] = yield Promise.all([
            ...
            const allTokensReceived = tokensAskedFor.every(fetchableToken => directTokens.some(fetchedToken => compareToken(fetchableToken, fetchedToken)));
            console.assert(allTokensReceived, "fetch bulk balances failed to return all tokens asked for!");
            const equalisedBalances = bonusTokens.tokens.map(tokenBalanceToTokenBalanceReturn);
            const merged = _.uniqWith([...directTokens, ...equalisedBalances], compareToken);
    ...
    updateTokenBalances(tokens) {
        const toSet = _.uniqWith([...tokens.map(pickBalanceReturn), ...this.tokenBalances], compareToken);
        this.tokenBalances = toSet;
    }
}
__decorate([
    action
], TlosNetworkModule.prototype, "pingTillChange", null);
__decorate([
    action
], TlosNetworkModule.prototype, "transfer", null);
__decorate([
    action
], TlosNetworkModule.prototype, "xtransfer", null);
__decorate([
    action
], TlosNetworkModule.prototype, "fetchBulkBalances", null);
__decorate([
    action
], TlosNetworkModule.prototype, "getBalances", null);
__decorate([
    mutation
], TlosNetworkModule.prototype, "updateTokenBalances", null);